<?php 
    ...
    if(isset($_POST['update'])) {
        ...
        if($imgPrin['error'] === UPLOAD_ERR_NO_FILE){
            $sqlUpdate = "UPDATE produtos SET nome= ?, descricao= ?, preco= ? WHERE idProdutos = ?";
            $stmt = $mysqli->prepare($sqlUpdate);
            $stmt->bind_param("ssdi", $nomeDoProduto, $descricao, $preco, $protocolo);
            $stmt->execute();
            $stmt->close();
