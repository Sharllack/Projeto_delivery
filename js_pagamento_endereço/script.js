...
let valorTotalOriginal = parseFloat(
  document
    .querySelector(".valorTotal")
    .textContent.replace("R$", "")
    .replace(",", ".")
);
let valorTaxaOriginal = parseFloat(
  document
    .querySelector(".valorTaxa")
    .textContent.replace("R$", "")
    .replace(",", ".")
);
...
document.querySelector("form").addEventListener("submit", function (event) {
  ...
  if (
    document.querySelector('input[name="opcTroco"]:checked')?.value === "sim"
  ) {
    if (!validarTroco()) {
      event.preventDefault();
      document.querySelector(".loading").style.display = "none";
...
document.querySelectorAll(".askTroco").forEach((item) => {
  item.addEventListener("change", function () {
    const askTr = document.querySelector(
      'input[name="opcTroco"]:checked'
    ).value;
    const troco = document.querySelector(".troco");
...
document.querySelector("#opcEntrega").addEventListener("change", function () {
  ...
  if (opc === "retirada") {
    const valorTotal = parseFloat(
      total.textContent.replace("R$", "").replace(",", ".")
    );
    const valorTaxa = parseFloat(
      taxa.textContent.replace("R$", "").replace(",", ".")
    );
    ...
    logra.textContent = "Rua Dr. Furquim Mendes, 990";
...
toggle.addEventListener("change", function () {
  if (this.checked) {
    ...
    img.src =
      "./imagens/location_on_24dp_E8EAED_FILL0_wght400_GRAD0_opsz24.png";
